...
TARGET_A_TYPE="tweeter.post"
TARGET_A_ID="post_${STAMP}"
TARGET_B_TYPE="youtube.video"
TARGET_B_ID="video_${STAMP}"
...
HTTP_CODE="$(curl -s -o /dev/null -w "%{http_code}" \
  "${BASE}/comments/targets/${TARGET_A_TYPE}/${TARGET_A_ID}")"
if [ "$HTTP_CODE" != "401" ]; then
  echo "Expected 401 for unauthenticated /comments, got ${HTTP_CODE}"
  exit 1
fi

echo "[4/8] Creating comments on two unrelated target namespaces..."
C1="oldest target-a comment ${STAMP}"
C2="middle target-a comment ${STAMP}"
C3="newest target-a comment ${STAMP}"
OTHER="target-b comment ${STAMP}"

FIRST_COMMENT="$(comment_as "$BOB_TOKEN" "$TARGET_A_TYPE" "$TARGET_A_ID" "$C1")"
...
comment_as "$BOB_TOKEN" "$TARGET_A_TYPE" "$TARGET_A_ID" "$C2" >/dev/null
comment_as "$BOB_TOKEN" "$TARGET_A_TYPE" "$TARGET_A_ID" "$C3" >/dev/null
comment_as "$ALICE_TOKEN" "$TARGET_B_TYPE" "$TARGET_B_ID" "$OTHER" >/dev/null

echo "[5/8] Reading comment by id..."
COMMENT_BODY="$(curl -fsS "${BASE}/comments/${COMMENT_ID}" \
  -H "Authorization: Bearer ${ALICE_TOKEN}")"
require_contains "$COMMENT_BODY" "$C1" "comment lookup"
require_contains "$COMMENT_BODY" "$TARGET_A_TYPE" "comment lookup"

echo "[6/8] Reading target comments with cursor paging and isolation..."
PAGE1="$(curl -fsS "${BASE}/comments/targets/${TARGET_A_TYPE}/${TARGET_A_ID}?pageSize=2" \
  -H "Authorization: Bearer ${ALICE_TOKEN}")"
require_contains "$PAGE1" "$C3" "first comment page"
...
PAGE2="$(curl -fsS "${BASE}/comments/targets/${TARGET_A_TYPE}/${TARGET_A_ID}?pageSize=2&cursor=${CURSOR}" \
  -H "Authorization: Bearer ${ALICE_TOKEN}")"
