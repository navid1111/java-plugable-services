...
class MediaUploadIntentServiceTest {
    ...
    void repeatedIdempotencyKeyReturnsSameIntentAndAbandonedIntentExpires() {
        ...
        var first = service.create("alice", "post", "42", "request-1", "image", "png", 1000).intent();
        when(intents.findByOwnerUsernameAndIdempotencyKey("alice", "request-1")).thenReturn(Optional.of(first));
        var repeated = service.create("alice", "post", "42", "request-1", "image", "png", 1000).intent();
        assertEquals(first.getId(), repeated.getId());
        verify(targets, times(1)).requireActiveOwnedBy("post", "42", "alice");

        MediaUploadIntent expired = new MediaUploadIntent(UUID.randomUUID(), "alice", "post", "42", "old",
    ...
    void forgedIdentityAndOversizedFinalizeAreRejectedBeforePersistence() {
        ...
        MediaUploadIntent intent = new MediaUploadIntent(UUID.randomUUID(), "alice", "post", "42", "key",
        ...
        assertThrows(IllegalArgumentException.class, () -> service.finalizeUpload(intent.getId(), "alice",
    ...
    void successfulAndFailedLifecyclesWriteTransactionalEvents() {
        ...
        service.finalizeUpload(intent.getId(), "alice", new MediaUploadIntentService.FinalizeRequest(
                "issued-id", "image", "png", "https://res.cloudinary.com/x.png", 100,
                null, null, null, "x.png"));
        verify(events, times(2)).write(any());

        MediaUploadIntent failed = new MediaUploadIntent(UUID.randomUUID(), "alice", "post", "42", "key2",
                "image", "png", 1000, "failed-id", Instant.now().plusSeconds(60));
        when(intents.findById(failed.getId())).thenReturn(Optional.of(failed));
        service.failUpload(failed.getId(), "alice", "provider unavailable");
