...
STAMP="$(date +%s)"
ALICE="turf_alice_${STAMP}"
BOB="turf_bob_${STAMP}"
PASS="smoke_pass"
WORK_DIR="${TMPDIR:-/tmp}/turf-smoke-${STAMP}"

echo "Running turf smoke test against ${BASE}"
...
HTTP_CODE="$(curl -s -o /dev/null -w "%{http_code}" "${BASE}/bookings/venues")"
if [ "$HTTP_CODE" != "401" ]; then
  echo "Expected 401 for unauthenticated /bookings/venues, got ${HTTP_CODE}"
  exit 1
fi

echo "[4/9] Browsing seeded venues and slots..."
VENUES="$(curl -fsS "${BASE}/bookings/venues" -H "Authorization: Bearer ${ALICE_TOKEN}")"
require_contains "$VENUES" "Northside Arena" "venues listing"
SLOT_ID="$(printf '%s' "$VENUES" | first_available_slot)"
if [ -z "$SLOT_ID" ]; then
...
VENUES_AFTER_BOOKING="$(curl -fsS "${BASE}/bookings/venues" -H "Authorization: Bearer ${ALICE_TOKEN}")"
AVAILABLE="$(printf '%s' "$VENUES_AFTER_BOOKING" | slot_available_state "$SLOT_ID")"
if [ "$AVAILABLE" != "false" ]; then
  echo "Expected booked slot ${SLOT_ID} to be unavailable"
  echo "$VENUES_AFTER_BOOKING"
  exit 1
fi
...
echo "Turf smoke test passed successfully."
